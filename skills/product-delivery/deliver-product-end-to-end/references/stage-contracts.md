...
| 状态 | 必需输入 | 必需 Evidence | 下一步审批主题 |
| --- | --- | --- | --- |
| `DISCOVERY` | 原始想法 | 原话记录、未知项 | 产品解释是否正确 |
| `PRODUCT_CONFIRMED` | Rxx、MVP、非目标、Product Experience Brief | 用户价值、核心任务、体验目标、按当前审批策略生成的批准记录、ACxx 草案 | 页面结构与旅程 |
...
| `TECH_PLAN_APPROVED` | 项目现状、选型、复用边界 | 按当前审批策略生成的批准记录、风险 | 前后端合同 |
...
- 表中的“审批主题”由当前审批策略处理：`INTERACTIVE` 交给用户，`DELEGATED_SUPERVISOR` 交给独立 Reviewer；`USER_ACCEPTED` 永远只能由用户确认。
