...
孵化 Chat：模糊 Prompt → 主动产品提问 → 项目确认
  ...
  → 新建执行 Chat
  → 选择逐阶段确认或 DELEGATED_SUPERVISOR 监工委托
  → 用户价值与体验方案
  → Logo、视觉和交互原型
  → 按审批策略审核确认 Figma
  → 技术方案与服务契约
...
用户选择 `DELEGATED_SUPERVISOR` 后，只需要给出需求并查看最终结果。独立 Reviewer 会审核产品、体验、品牌、Figma、技术、合同、实现和测试，不通过则自动返回原 Worker 修正；OAuth、付费、生产、不可逆动作、未预授权 Git 和最终验收仍由用户确认。
